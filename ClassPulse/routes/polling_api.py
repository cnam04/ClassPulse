...
@polling_bp.post("/api/session/<code>/poll/start")
def api_poll_start(code):
    q = (request.json or {}).get("question", "")
    poll_start(code, q)
    return jsonify({"ok": True})

@polling_bp.post("/api/session/<code>/poll/stop")
def api_poll_stop(code):
    poll_stop(code)
    return jsonify({"ok": True})

@polling_bp.get("/api/session/<code>/poll")
...
@polling_bp.post("/api/session/<code>/poll/vote")
